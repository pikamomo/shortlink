...
package com.chi.shortlink.project.mq.consumer;
...
import static com.chi.shortlink.project.common.constant.RedisKeyConstant.LOCK_GID_UPDATE_KEY;
import static com.chi.shortlink.project.common.constant.ShortLinkConstant.AMAP_REMOTE_URL;

/**
 * Short link monitoring status save message queue consumer
 *
 */
@Slf4j
@Component
...
public class ShortLinkStatsSaveConsumer implements StreamListener<String, MapRecord<String, String, String>> {
    ...
    public void onMessage(MapRecord<String, String, String> message) {
        ...
        if (messageQueueIdempotentHandler.isMessageBeingConsumed(id.toString())) {
            // Determine if the current message process is complete
            if (messageQueueIdempotentHandler.isAccomplish(id.toString())) {
                return;
            }
            throw new ServiceException("Message process not completed, message queue needs to retry");
        }
        try {
            ...
            // Certain situation crashed
            messageQueueIdempotentHandler.delMessageProcessed(id.toString());
            log.error("Exception in recording short link monitoring consumption", ex);
            throw ex;
        }
